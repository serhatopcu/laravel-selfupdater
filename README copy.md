...
```sh
$ composer require serhatopcu/laravel-selfupdater
```

## Configuration
After installing the package you need to publish the configuration file via
```sh
$ php artisan vendor:publish --provider="SerhaTopcu\Updater\UpdaterServiceProvider"
```
...
```php
Route::get('/', function (\SerhaTopcu\Updater\UpdaterManager $updater) {
...
Select the branch that should be used via the `use_branch` setting [inside the configuration](https://github.com/serhatopcu/laravel-selfupdater/blob/master/config/self-update.php).
