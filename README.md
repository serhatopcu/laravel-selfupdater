...
```sh
$ composer require serhattopcu/laravel-selfupdater
```

## Configuration
After installing the package you need to publish the configuration file via
```sh
$ php artisan vendor:publish --provider="SerhatTopcu\Updater\UpdaterServiceProvider"
```
...
```php
Route::get('/', function (\SerhatTopcu\Updater\UpdaterManager $updater) {
...
Select the branch that should be used via the `use_branch` setting [inside the configuration](https://github.com/serhattopcu/laravel-selfupdater/blob/master/config/self-update.php).
